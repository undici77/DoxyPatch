...
class DoxygenCCPP
{
	...
	private static readonly string _Constructor_Regex = RegexBuilder.Make(new RegexString("^") +
																		  ...
																		  new RegexCapturePadding() +
																		  new RegexTemplate() +
																		  new RegexCPPConstructorExcludeKeywork() +
																		  new RegexCPPCaptureConstructorName() +
	...
	private static readonly string _Function_Regex = RegexBuilder.Make(new RegexString("^") +
																	   ...
																	   new RegexCapturePadding() +
																	   new RegexTemplate() +
																	   new RegexCPPMethodExcludeKeywork() +
																	   new RegexCPPCaptureMethodName() +
																	   new RegexCaptureParameters() +
																	   new RegexCaptureUntilBodyBegin());
	...
	/// @brief Initializes a new instance of the DoxygenCCPP class.
	///        This constructor initializes a new instance of the DoxygenCCPP class with the specified OllamaClient.
	///
	/// @param ollama_client The OllamaClient to be used by this instance.
	public DoxygenCCPP(OllamaClient ollama_client)
	{
		_Ollama_Client = ollama_client;
	}

	/// @brief Verifies and generates Doxygen comments for a given C# file.
	///        This method processes a specified C# file to verify existing Doxygen comments and generate new ones if necessary.
	///        It handles constructors and functions, optionally using an external service (Ollama) to enhance comment generation.
	///
	/// @param file_name The path to the C# file to be processed.
	/// @param ollama A flag indicating whether to use Ollama for generating Doxygen comments.
	/// @param rebuild A flag indicating whether to rebuild existing Doxygen blocks.
	/// @param dry_mode A flag indicating whether to perform a dry run without writing changes to the file.
	/// @param with_context A flag indicating whether to set context using Ollama before processing.
	public void VerifyAndGenerate(string file_name, bool ollama, bool rebuild, bool dry_mode, bool with_context)
	{
		...
		do
		{
			...
			else
			{
				...
				if (doxygen_generic_match.Success)
				{
				...
				else if (ConstructorMatch(constructor_match))
				{
					...
					if (AnalyzeDoxygenCommentBlock(constructor_match, ref doxygen_block.data, out error_log, out warning_log, out return_value, out method_name))
					{
						if (ollama)
						{
							try
							{
								var ollama_doxygen_comment_block = _Ollama_Client.GenerateDoxygen(constructor_match.Groups[0].Value, "C++", body).GetAwaiter().GetResult();
				...
				else if (function_match.Success)
				{
					...
					if (AnalyzeDoxygenCommentBlock(function_match, ref doxygen_block.data, out error_log, out warning_log, out return_value, out method_name))
					{
						if (ollama)
						{
							try
							{
								var ollama_doxygen_comment_block = _Ollama_Client.GenerateDoxygen(function_match.Groups[0].Value, "C++", body).GetAwaiter().GetResult();
								doxygen_block.data = Utilities.MergeDoxygenHeader(doxygen_block.data, ollama_doxygen_comment_block);
	...
	/// @brief Analyzes a Doxygen comment block for a given method and updates the comments if necessary.
	///        This method processes a provided Match object containing potential Doxygen comments, checks for the presence of required tags,
	///        and ensures that all parameters and return values are correctly documented. It logs errors and warnings for missing or incorrect
	///        documentation and can generate a new skeleton for empty Doxygen headers.
	///
	/// @param input_match The Match object containing the method code and its associated comments.
	/// @param doxygen_comments A reference to the string holding the current Doxygen comments, which may be updated by this method.
	/// @param error_log An output list of strings where errors related to missing or incorrect documentation are logged.
	/// @param warning_log An output list of strings where warnings related to empty or potentially incorrect documentation are logged.
	/// @param return_value An output string representing the return type of the analyzed function, extracted from the method code.
	/// @param name An output string representing the name of the analyzed function, extracted from the method code.
	/// @retval A boolean indicating whether the Doxygen comments were updated and need to be written back to a file.
	...
	/// @brief Determines if a given Match object is successful.
	///        This function checks the Success property of the provided Match object to determine if it was successful.
	///
	/// @param match The Match object to check for success.
	/// @retval true If the Match object was successful.
	///         false If the Match object was not successful.
