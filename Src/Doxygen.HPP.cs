...
class DoxygenHPP
{
	...
	private static readonly string _Constructor_Regex = RegexBuilder.Make(new RegexString("^") +
																		  ...
																		  new RegexCapturePadding() +
																		  new RegexTemplate() +
																		  new RegexHPPConstructorExcludeKeywork() +
																		  new RegexHPPConstructorAccessModifier() +
																		  new RegexHPPCaptureConstructorName() +
	...
	private static readonly string _Function_Regex = RegexBuilder.Make(new RegexString("^") +
																	   ...
																	   new RegexCapturePadding() +
																	   new RegexTemplate() +
																	   new RegexHPPMethodExcludeKeywork() +
																	   new RegexHPPCaptureMethodName() +
																	   new RegexCaptureParameters() +
																	   new RegexCaptureUntilBodyBegin());
	...
	/// @brief Initializes a new instance of the DoxygenHPP class.
	///        This constructor initializes a new instance of the DoxygenHPP class with the specified OllamaClient.
	///
	/// @param ollama_client The OllamaClient to be used by this instance.
	public DoxygenHPP(OllamaClient ollama_client)
	{
		_Ollama_Client = ollama_client;
	}

	/// @brief Verifies and generates Doxygen comments for a given C++ file.
	///        This method processes a specified C++ source file to verify the presence of Doxygen comments
	///        and generate them if necessary. It handles constructors and functions, optionally using an external
	///        service (Ollama) to enhance or create Doxygen blocks. The method also supports options for rebuilding,
	///        running in dry mode, and including context.
	///
	/// @param file_name The path to the C++ source file to process.
	/// @param ollama A flag indicating whether to use Ollama for generating enhanced Doxygen comments.
	/// @param rebuild A flag indicating whether to rebuild existing Doxygen blocks.
	/// @param dry_mode A flag indicating whether to run in dry mode (no actual file modifications).
	/// @param with_context A flag indicating whether to include context when using Ollama.
	public void VerifyAndGenerate(string file_name, bool ollama, bool rebuild, bool dry_mode, bool with_context)
	{
		...
		do
		{
			...
			else
			{
				...
				var class_name_match = new Regex(RegexBuilder.CPPClassStructNamespaceRegex(), RegexOptions.None, TimeSpan.FromSeconds(5)).Match(input);
					...
					if (AnalyzeDoxygenCommentBlock(constructor_match, ref doxygen_block.data, out error_log, out warning_log, out return_value, out method_name))
					{
						if (ollama)
						{
							try
							{
								...
								method = constructor_match.Groups[0].Value;
								if (class_name != string.Empty)
								{
									method = method.Replace(method_name, class_name + "::" + method_name);
								}

								var ollama_doxygen_comment_block = _Ollama_Client.GenerateDoxygen(method, class_name, "C++", body).GetAwaiter().GetResult();
				...
				else if (function_match.Success)
				{
...
#if DEBUG
					Log.Instance.AppendEvent(class_name + " - " + function_match.Value);
#endif
					if (rebuild)
					{
						doxygen_block.data = "";
					...
					if (AnalyzeDoxygenCommentBlock(function_match, ref doxygen_block.data, out error_log, out warning_log, out return_value, out method_name))
					{
						if (ollama)
						{
							...
							{
								string method = string.Empty;
								method = function_match.Groups[0].Value;
								if (class_name != string.Empty)
								{
									method = method.Replace(method_name, class_name + "::" + method_name);
								}

								var ollama_doxygen_comment_block = _Ollama_Client.GenerateDoxygen(method, "C++", body).GetAwaiter().GetResult();
								doxygen_block.data = Utilities.MergeDoxygenHeader(doxygen_block.data, ollama_doxygen_comment_block);
	...
	///        This function evaluates whether a given Match object represents a valid constructor match
	///        by checking if the third group in the match is either null or an empty string.
	///
	/// @param match The Match object to evaluate.
	/// @retval True if the match is considered a valid constructor match, false otherwise.
